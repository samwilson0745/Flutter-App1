...
import 'package:myapptest/Admin/Panel/uploadImagePanel.dart';


class Upload extends StatefulWidget {
  String apiToken;
  Upload(this.apiToken);
  @override
...
class _UploadState extends State<Upload> {
  ...
  bool _checkStudent(String Name,String classId){
    for(i=0;i!=_allStudent.length;i++){
      if(Name==_allStudent[i]['Name'] && classId==_allStudent[i]['ClassId']){
        Navigator.push(context, MaterialPageRoute(builder: (context) => UploadImage(_allStudent[i],widget.apiToken)) );
        return true;
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    _checkStudent(_nameController.text,classId);
    }
    },
    child: Text('Search')
    ),]
